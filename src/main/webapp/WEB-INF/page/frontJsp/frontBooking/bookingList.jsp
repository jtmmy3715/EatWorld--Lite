...
<body>
	<div class="main-wrapper wrapper-2">
		...
		<div class="breadcrumb-area pt-95 pb-100 bg-img"
			style="background-image:url(<%=request.getContextPath()%>/resources/assets/images/bg/breadcrumb-booking.jpg);">
			<div class="container">
				<div class="breadcrumb-content text-center">
					<div class="breadcrumb-title">
						<!-- 標題行 -->
						<h2 style="gray">訂位店家列表</h2>
					</div>
					<ul>
					...
					<div class="col-lg-9">
						...
						<div class="shop-top-bar">
							<div class="select-shoing-wrap">
								...
								<p>顯示 1–12 共 <span id="store_count"><c:out value="${storeDetails.count}"/></span> 結果</p>
							</div>
												...
												<div class="product-wrap product-border-1 mb-30">
													...
													<div class="product-img" style="text-align: center;">
														<a href="#"> <img
															...
															alt="store" style="height:180px; width:auto;"></a>
														<span class="product-badge" id="store_label">New</span>
												...
												<div class="col-xl-4 col-lg-5 col-md-5 col-sm-6">
													<div class="product-wrap product-border-1" style="text-align: center;">
														<div class="product-img">
															<a href=" #"><img
																src="../../../photo/${store.store_logo}"
																alt="store" style="height:200px; width:auto;"></a>
															<c:if test="${store.store_people > 0}">
															</c:if>
															<c:if test="${store.store_people <= 0}">
															<span class="product-badge" id="store_label">
															訂位已滿
															</span>
															</c:if>
															<div class="product-action">
													...
													<div class="shop-list-content">
														...
														<div class="shop-list-btn-wrap">
															...
															<div class="shop-list-wishlist default-btn btn-hover">
																<a href="#"><i class="la la-heart-o" onclick="balanceBooking(1);"></i></a>
															</div>
														</div>
													</div>
												</div>
							...
							<div class="sidebar-widget">
								...
								<div class="sidebar-widget-list mt-30" id="store_search_check">
									<ul>
										...
										<li>
											<div class="sidebar-widget-list-left">
												<input type="checkbox" value="尚有訂位" id="store_search_check_1">
												<a href="#">尚有訂位</a>
										...
										<li>
											<div class="sidebar-widget-list-left">
												<input type="checkbox" value="未有訂位" id="store_search_check_0">
												<a href="#">未有訂位</a>
												<span class="checkmark"></span>
											</div>
										</li>
												...
												<div class="pro-details-size">
													...
													<input type="hidden" name="booking_period" id="booking_period_val">
														<ul id=booking_period>
															<li><a href="###" id="booking_period1">上午</a></li>
															<li><a href="###" id="booking_period2">中午</a></li>
															<li><a href="###" id="booking_period3">晚上</a></li>
														</ul>
													</div>
												</div>
												<div class="pro-details-color-wrap">
											...
											<div class="pro-details-quality">
												...
												<div class="pro-details-cart btn-hover">
													<button id="form_button_submit" type="submit"
														name="form_button_submit" onclick="send();">送出</button>
												</div>
												<div class="pro-details-wishlist">
													<a title="收藏店家" href="#"><i class="la la-heart-o"></i></a>
												</div>
	...
	<!-- 監聽 訂位時段按紐 -->
	$('#booking_period li a').on('click',function(){
		booking_period_click = $(this).text();
		$('#booking_period_val').val(booking_period_click);
		console.log('選擇時段: ' + booking_period_click);
		
		if(booking_period_click=='上午'){
			$('#booking_period1').focus();
		}else if(booking_period_click=='中午'){
			$('#booking_period2').focus();
		}else if(booking_period_click=='晚上'){
			$('#booking_period3').focus();
		}
		
		//
		balanceBooking();
	});
	$('#booking_period li a').blur(function(){
		if(booking_period_click=='上午'){
			$('#booking_period1').focus();
		}else if(booking_period_click=='中午'){
			$('#booking_period2').focus();
	...
	$('#booking_date').on('click',function(){
		...
		console.log('今天日期: ' + year + '-' + month + '-' + d);
			$(this).attr('min',year + '-' + month + '-' + d);
			
		//
		
	});
	$('#booking_date').on('change',function(){
		console.log('選擇日期: ' + $('#booking_date').val() );
		balanceBooking();
	});
	...
	function storelist(key){
		...
		$.ajax({
			...
			success: function(data){
				...
				console.log('長度: ' + Object.keys(data).length);
				var store_count = Object.keys(data).length;
				...
				var label = '';
				if(n.store_booking == 0){
					}else if(n.store_people <= 0){
					var label = '<span class="product-badge">訂位已滿</span>';
					}
				...
				result = result
					...
					+	'<div class="product-wrap product-border-1" style="text-align: center;">'
					+	'<div class="product-img">'
					+	'<a href=" #"><img src="../../../photo/' + n.store_logo +'" alt="store" style="height:200px; width:auto;"></a>'
					+	label
					+	'<div class="product-action">'
	...
	<!-- 查詢剩餘訂位 -->
	function balanceBooking(){
		var date = $('#booking_date').val();
		var period = booking_period_click;
		...
		$.ajax({
			type: 'post',
			url: '<%=request.getContextPath()%>/front/booking/bookingPeopleBalance',
			dataType: 'text',
			contentType: 'application/json',
			data : JSON.stringify(params),
			success : function(data) {
				console.log(date + ',' + period + ',' + data);
				$('#store_booking').text(data);				
			},
			error: function(e){
				console.log('error');
				console.log(e);
	...
	function getStore(sid){
		$.ajax({
			...
			success: function(data){
				...
				if(data.store_people == '0'){
				$('#store_booking').text('今日已無訂位');
				//$('#store_label').text('今日已無訂位');
				}else{
					$('#store_booking').text('剩餘訂位人數: ' + data.store_people)
					//$('#store_label').text('剩餘訂位人數: ' + data.store_people)
					};
			}
	...
	$('#form').submit(function(){
		...
		var store_people = $('#store_booking').text();
		
		//debug
		console.log("store_id: " + store_id, "client_id: " + client_id + "\n", 
				"booking_people: " + booking_people, "booking_date: " + booking_date + "\n", 
				"booking_period: " + booking_period, "store_people: " + store_people);
		
		if(store_people == '今日已無訂位' || store_people == '此店家已無訂位!'){
			Swal.fire({
				icon: 'error',
				title: '今日已無訂位',
			});
		...
		if(booking_period == '' || booking_period == null){
			Swal.fire({
				icon: 'error',
				title: '請選擇時段',
			});
